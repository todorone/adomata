...
This Cloudflare Worker calls the API's authenticated `/heartbeat` endpoint every
five minutes. It is deployed separately from the client SPA.

Set `HEARTBEAT_SECRET` as a Cloudflare Worker secret to the same value used by
