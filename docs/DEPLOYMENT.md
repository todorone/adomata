...
- **API** (`apps/api`) runs in Docker on the **same shared Hetzner VPS/Coolify
  instance as FrontPeek** (`78.46.206.9`, hostname `frontpeek`, ARM64 Ubuntu),
  managed by **Coolify** (self-hosted PaaS), as its own separate app grouping.
  Public origin: `https://api.adomata.com`.
...
> ⚠️ **The VPS is shared** — with FrontPeek and other unrelated production
> services (WordPress + MariaDB, Umami, Uptime-Kuma, and Coolify itself). Only
> ever touch Adomata's own containers. Coolify assigns each app's containers a
> generic `api-*` / `db-*` name plus a per-app UUID, so Adomata's containers
> carry the **same prefixes as FrontPeek's** but a different UUID — confirm the
> exact UUID for Adomata's Coolify resource before selecting containers (see
> below), and never touch a container you haven't confirmed belongs to Adomata.
...
Production migration safety is fail-closed (mirroring FrontPeek's
`docs/adr/0006-production-migration-recovery.md` — no separate ADR for this
in Adomata, this doc is the source of truth):
...
(FrontPeek's containers use the same prefixes). **Confirm Adomata's specific
UUID in the Coolify dashboard first**, then select by that UUID-qualified
prefix — never hardcode a full container name, and never select by the bare
`api-`/`db-` prefix alone on this host:

```sh
api=$(ssh root@78.46.206.9 'docker ps -q --filter name=^api-<adomata-uuid>')
db=$(ssh root@78.46.206.9  'docker ps -q --filter name=^db-<adomata-uuid>')
```

### Logs

```sh
ssh root@78.46.206.9 'docker logs --tail 200 <adomata-api-container>'
```
...
account (`pmahotsava@gmail.com` — see `SUPERADMIN_EMAIL` in `apps/api/.env.example`;
credentials themselves live wherever the maintainer keeps prod secrets, not in
this repo) and call the API with the returned bearer token. Prefer this
black-box path over the DB for verifying behavior.
...
```sh
ssh root@78.46.206.9 'docker exec -i <adomata-db-container> \
...
2. **Never touch non-Adomata containers** — this includes FrontPeek's `api-*` /
...
- **Cloudflare** — Workers project for the `app.adomata.com` client.
